...
class RuntimeChecks(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.records = json.loads((OLD / 'prepared_dev.json').read_text())
        cls.results = json.loads((OLD / 'audit_results.json').read_text())
    ...
    def invoke(self, model=None, sink=None, score=None, records=None):
        ...
        if not runtime._PREFLIGHTS.get(model, {}).get('passed'):
            runtime.real_runtime_context_preflight(model, torch)
    ...
    def test_02_adapter_binding(self):
        binding = json.loads((OLD / 'adapter_binding.json').read_text())
        remote = json.loads((ROOT / 'docs/fix/producer_checkpoint120_eval_runtime_v2/downloaded/evidence/peft_preflight.json').read_text())
        self.assertEqual(binding['files'], self.protocol['adapter']['files'])
        self.assertEqual(remote['adapter_hashes'], binding['files'])
        self.assertTrue(remote['adapter_tensor_identity'])
        self.assertEqual(binding['files']['adapter_model.safetensors'], '3f1e12d107f1206641902565b28ea6217d70ff2768c321a216227dbc21b702a6')
    ...
    def test_29_authorization_fail_closed(self):
        ...
        valid = dict(operator_authorized=True, experiment='second-tuning-eval-runtime-v2_1', action='producer-checkpoint120-evaluation-only', role='producer', split='canonical', checkpoint=120, runtime_release_sha256='release', protocol_sha256=runtime.digest(self.protocol), adapter_sha256=self.protocol['adapter']['files']['adapter_model.safetensors'])
    ...
    def session(self):
        permit = dict(operator_authorized=True, experiment='second-tuning-eval-runtime-v2_1', action='producer-checkpoint120-evaluation-only', role='producer', split='canonical', checkpoint=120, runtime_release_sha256='test-only-release', protocol_sha256=runtime.digest(self.protocol), adapter_sha256=self.protocol['adapter']['files']['adapter_model.safetensors'])
        session = runtime.EvaluationSession(permit, self.protocol, 'test-only-release')
        session.preflight_context(Model(), TorchFacade())
        return session
    ...
    def test_45_session_denies_stitching_and_resume(self):
        ...
        session.admit_control(self.control_rows(1), self.control_rows(.2))
        model = session.context_model
        with self.assertRaisesRegex(ValueError, 'Wrong DEV population/order'):
    ...
    def test_46_session_denies_protocol_drift(self):
        ...
        with self.assertRaises(ValueError):
            session.complete_dev(session.context_model, None, TorchFacade(), self.records, Sink(), None)

"""Authorized final Auditor workflow; one A10, $5 soft / $7 hard ceiling."""
...
ROOT=Path(__file__).resolve().parents[1];BASE=ROOT/'docs/fix/auditor_final_retry_run';FROZEN=ROOT/'tuning/second_domain_agnostic_v2'
CREDENTIAL=Path('C:/Users/secki/local/api_keys/config.py');REMOTE='/home/ubuntu/shimmer-auditor-final'
...
def prepare():
    from prepare_auditor_final_retry import prepare as build
    build()
...
def execute():
    ...
    require(read(BASE/'LOCAL_GATES.json')['execution_manifest_sha256']==sha(BASE/'execution_manifest.json'),'Tested manifest changed')
    require(all(sha(ROOT/name)==digest for name,digest in manifest['local_source_hashes'].items()),'Reviewed source changed')
    require(m['region']=='us-east-1' and m['instance']['type']=='gpu_1x_a10' and 0<m['hourly_rate']<=1.29 and m['soft_usd']==5 and m['hard_usd']==7,'Authorized resource caps')
    require(m['workload_deadline_seconds']<=7/m['hourly_rate']*3600-900 and m['watchdog_deadline_seconds']<=7/m['hourly_rate']*3600-180,'Teardown reserves')
    permit=read(BASE/'training_authorization.json')
    require(permit.get('operator_authorized') is True and permit.get('action')=='auditor-final-training-evaluation','Explicit retry authorization required')
    require(permit['execution_manifest_sha256']==sha(BASE/'execution_manifest.json') and permit['seal_sha256']==manifest['seal_sha256'] and permit['source_commit']==m['source_commit'],'Authorization identity')
    ...
    require(m['image'] in provider.request('images')['data'],'Pinned image unavailable')
    ...
    def remote(label,command,timeout=60,required=True):
        ...
        if time.time()>=start+m['soft_seconds']:
            ...
            remaining_work=900 if (BASE/'TRAINING_RELEASE.json').exists() else 7000
    ...
    try:
        ...
        with (BASE/'LAUNCH_INTENT.json').open('x') as lock:json.dump(dict(epoch=start,maximum_instances=1,authorization='explicit final896 delayed DEV; one instance; soft5 hard7'),lock)
        ...
        permit=read(BASE/'training_authorization.json');permit.update(launch_epoch=start,workload_deadline_epoch=deadline,hourly_rate=m['hourly_rate']);write('training_authorization.json',permit)
        ...
        transport('runtime_transfer',['scp',*options,str(BASE/'runtime_bundle.zip'),str(BASE/'training_authorization.json'),host+':'+REMOTE+'/'],120)
        ...
        remote('pre_acquisition',prefix+exe+' -B tools/auditor_final_launch_support.py preflight',120)
        remote('acquisition',prefix+exe+' -B tools/auditor_final_launch_support.py acquire',1800)
        ...
        result=remote('final_training',prefix+env+exe+' -B -u tools/auditor_final_remote.py training',22000,False)
        require(result.returncode==0,'Final training failed; no evaluation, rescue or second instance')
        receipt=transport('training_completion',ssh+['cat '+REMOTE+'/evidence/TRAINING_COMPLETE.json'],30)
        completion=json.loads(receipt.stdout)
        from auditor_final_core import admit_evaluation,EVAL_HASHES
        admit_evaluation(completion)
        require(completion['source_commit']==m['source_commit'] and completion['seal_sha256']==permit['seal_sha256'],'Training completion identity')
        write('TRAINING_RELEASE.json',dict(epoch=time.time(),completion=completion,evaluation_payload_not_previously_opened=True))
        # The original DEV/challenge bytes are first opened here, only after verified update896.
        for name,digest in EVAL_HASHES.items():
            path=ROOT/'tuning/auditor_classifier_lora'/name
            require(sha(path)==digest,'Frozen evaluation payload changed')
            require(not credential_locations(path.read_bytes(),name),'Possible credential in evaluation payload; stop')
        remote('evaluation_mkdir','mkdir -p '+REMOTE+'/evaluation_payload')
        transport('evaluation_transfer',['scp',*options,*[str(ROOT/'tuning/auditor_classifier_lora'/name) for name in EVAL_HASHES],host+':'+REMOTE+'/evaluation_payload/'],180)
        result=remote('final_evaluation',prefix+env+exe+' -B -u tools/auditor_final_remote.py evaluation',2400,False)
        require(result.returncode==0,'Final evaluation failed; no rescue or second instance')
        write('MANDATORY_WORK_FINISHED.json',dict(epoch=time.time(),final_training_and_evaluation=True))
        ...
        if iid:
            if ssh:
                try:
                    transport('stop_workloads',ssh+['pkill -TERM -f "[t]ools/auditor_final_remote.py" || true'],30,False)
                    transport('process_final',ssh+['nvidia-smi --query-compute-apps=pid,process_name,used_memory --format=csv,noheader'],30,False)
                    transport('pack_evidence',ssh+['cd '+REMOTE+' && tar -czf evidence.tar.gz evidence execution_manifest.json training_authorization.json budget.json tools tuning/auditor_final tuning/first_domain_agnostic_v1'],120,False)
...
def status():
    for name in ('CURRENT_PHASE.json','cost.json','auditor_evaluation_timing.json','control_summary.json','TERMINATION_VERIFIED.json','controller_failure.json'):

"""One authorized evaluation-only instance; no training, Auditor, or protected work."""
...
ROOT=Path(__file__).resolve().parents[1];BASE=ROOT/'docs/fix/producer_checkpoint120_eval_v2_1';FROZEN=ROOT/'tuning/second_domain_agnostic_v2'
CREDENTIAL=Path('C:/Users/secki/local/api_keys/config.py');REMOTE='/home/ubuntu/shimmer-checkpoint120-v21'
...
def prepare():
    from checkpoint120_v21_prepare import prepare as build
    build()
...
def execute():
    ...
    try:
        ...
        write('LAUNCH_INTENT.json',dict(epoch=start,maximum_instances=1,request_attachment='9ec2056f-90b3-4950-b45b-e7915a487b60'))
        ...
        transport('runtime_transfer',['scp',*options,str(BASE/'runtime_bundle.zip'),str(BASE/'evaluation_authorization.json'),host+':'+REMOTE+'/'],120)
        ...
        remote('pre_acquisition',prefix+exe+' -B tools/checkpoint120_v21_remote.py preflight',120)
        remote('acquisition',prefix+exe+' -B tools/checkpoint120_v21_remote.py acquire',1800)
        ...
        result=remote('producer_evaluation',prefix+env+exe+' -B -u tools/checkpoint120_v21_remote.py execute',7200,False)
        ...
        if iid:
            if ssh:
                try:
                    transport('stop_workloads',ssh+['pkill -TERM -f "[t]ools/checkpoint120_v21_remote.py" || true'],30,False)
                    transport('process_final',ssh+['nvidia-smi --query-compute-apps=pid,process_name,used_memory --format=csv,noheader'],30,False)
                    transport('pack_evidence',ssh+['cd '+REMOTE+' && tar -czf evidence.tar.gz evidence execution_manifest.json evaluation_authorization.json budget.json tools/checkpoint120_v21_remote.py tuning/second_tuning_eval_runtime_v2_1/freeze.json'],120,False)

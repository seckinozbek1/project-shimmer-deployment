...
ROOT=Path(__file__).resolve().parents[1];BASE=ROOT/'docs/fix/auditor_final_run';FROZEN=ROOT/'tuning/second_domain_agnostic_v2'
...
def prepare():
    from prepare_auditor_final import bundle as build
    build()
        ...
        if iid:
            if ssh:
                try:
                    ...
                    transport('pack_evidence',ssh+['cd '+REMOTE+' && tar -czf evidence.tar.gz evidence execution_manifest.json training_authorization.json budget.json tools tuning/auditor_final'],120,False)

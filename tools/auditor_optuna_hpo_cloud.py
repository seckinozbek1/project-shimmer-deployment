"""Authorized ea2b723 clean HPO; one A10, $7 soft stop / $9 hard ceiling."""
...
ROOT=Path(__file__).resolve().parents[1];BASE=ROOT/'docs/fix/auditor_optuna_hpo_run';FROZEN=ROOT/'tuning/second_domain_agnostic_v2'
CREDENTIAL=Path('C:/Users/secki/local/api_keys/config.py');REMOTE='/home/ubuntu/shimmer-auditor-optuna-hpo'
...
def prepare():
    from prepare_auditor_optuna_hpo_launch import prepare as build
    build()
...
def execute():
    ...
    try:
        ...
        with (BASE/'LAUNCH_INTENT.json').open('x') as lock:json.dump(dict(epoch=start,maximum_instances=1,authorization='explicit clean HPO at ea2b723; one instance; soft7 hard9'),lock)
        ...
        deps=dict(read(ROOT/'tuning/auditor_optuna_hpo/runtime_contract.json')['packages'],**read(ROOT/'tuning/auditor_optuna_hpo/optuna_dependencies.json'))
        ...
        remote('pre_acquisition',prefix+exe+' -B tools/auditor_optuna_hpo_launch_support.py preflight',120)
        remote('acquisition',prefix+exe+' -B tools/auditor_optuna_hpo_launch_support.py acquire',1800)
        ...
        result=remote('classification_only',prefix+env+exe+' -B -u tools/auditor_optuna_hpo_remote.py execute --permit training_authorization.json --base-path base_model --output evidence',22000,False)
        require(result.returncode==0,'Evaluation runtime failed; no rescue or second instance')
        write('MANDATORY_WORK_FINISHED.json',dict(epoch=time.time(),hpo_only=True))
        ...
        if iid:
            if ssh:
                try:
                    transport('stop_workloads',ssh+['pkill -TERM -f "[t]ools/auditor_optuna_hpo_remote.py" || true'],30,False)
                    transport('process_final',ssh+['nvidia-smi --query-compute-apps=pid,process_name,used_memory --format=csv,noheader'],30,False)
                    transport('pack_evidence',ssh+['cd '+REMOTE+' && tar -czf evidence.tar.gz evidence execution_manifest.json training_authorization.json budget.json tools tuning/auditor_optuna_hpo'],120,False)

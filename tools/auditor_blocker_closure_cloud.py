"""Separately authorized closure diagnostic only. Preparation never authorizes provisioning."""
...
ROOT=Path(__file__).resolve().parents[1];BASE=ROOT/'docs/fix/auditor_blocker_closure_run';FROZEN=ROOT/'tuning/second_domain_agnostic_v2'
...
def prepare():
    from prepare_auditor_blocker_closure import prepare as build
    build()
...
def execute():
    ...
    require(read(BASE/'LOCAL_GATES.json')['execution_manifest_sha256']==sha(BASE/'execution_manifest.json'),'Tests do not bind this experiment')
    ...
    require(all(sha(ROOT/name)==digest for name,digest in read(BASE/'execution_manifest.json')['local_source_hashes'].items()),'Prepared source changed')
    ...
    permit=read(BASE/'diagnostic_authorization.json')
    require(permit.get('operator_authorized') is True and permit.get('action')=='auditor-blocker-closure-two-process-only','Separate operator authorization required before provisioning')
    require(permit['execution_manifest_sha256']==sha(BASE/'execution_manifest.json') and permit['arms']==['O','D'] and permit['max_rows_per_arm']==32,'Authorized experiment identity')
    ...
    require(m['image'] in provider.request('images')['data'],'Pinned image changed or unavailable')
    ...
    try:
        ...
        with (BASE/'LAUNCH_INTENT.json').open('x') as lock:json.dump(dict(epoch=start,maximum_instances=1,authorization='separately authorized O/D closure diagnostic; one instance; soft1 hard2; no training'),lock)
        ...
        remote('pre_acquisition',prefix+exe+' -B tools/auditor_blocker_support.py preflight',120)
        remote('acquisition',prefix+exe+' -B tools/auditor_blocker_support.py acquire',1800)
        ...
        result=remote('feature_diagnostic',prefix+env+exe+' -B -u tools/auditor_blocker_support.py run',3600,False)
        ...
        if iid:
            if ssh:
                try:
                    transport('stop_workloads',ssh+['pkill -TERM -f "[t]ools/auditor_blocker_(support|arm).py" || true'],30,False)
                    transport('process_final',ssh+['nvidia-smi --query-compute-apps=pid,process_name,used_memory --format=csv,noheader'],30,False)
                    transport('pack_evidence',ssh+['cd '+REMOTE+' && tar -czf evidence.tar.gz evidence execution_manifest.json diagnostic_authorization.json budget.json tools tuning/auditor_final closure_payload'],120,False)

...
def prepare():
    ...
    protocol=read(RT/'protocol.json');records=read(RT/'prepared_dev.json')
    ...
    scope=dict(experiment='second-tuning-eval-runtime-v2',action='producer-checkpoint120-evaluation-only',operator_authorized=True,
        role='producer',split='canonical',checkpoint=120,runtime_release_sha256=proof['runtime_release_sha256'],
        ...
        request_attachment='0a58459f-6387-45d5-8525-9965c91848ba',historical_permits_reused=False)
    ev.authorize(scope,protocol,proof['runtime_release_sha256'])
    ...
    names=['tools/checkpoint120_remote.py','tools/second_tuning_remote.py',
    ...
    names+=['tuning/second_tuning_eval_runtime_v2/'+n for n in ('eval_runtime.py','audit.py','telemetry_worker.py','protocol.json','prepared_dev.json','freeze.json')]
    ...
    execution=dict(source_commit=head,runtime_release_sha256=proof['runtime_release_sha256'],files={k:hashlib.sha256(v).hexdigest() for k,v in blobs.items()},
    ...
    manifest=dict(source_commit=head,release_sha256=proof['runtime_release_sha256'],bundle_sha256=sha(BASE/'runtime_bundle.zip'),
        ...
        watchdog_deadline_seconds=3/rate*3600-60,projected_seconds_with_reserve=projected,projected_cost=projected*rate/3600,name='shimmer-checkpoint120-eval')

"""Operational preflight/acquisition only. Does not modify the approved HPO workload."""
import argparse
...
def preflight():
    from auditor_optuna_hpo_remote import scope,authorize
    authorize(read(ROOT/'training_authorization.json'),h.sha(D/'seal.json'))
    ...
    pins=dict(contract['packages'],**read(D/'optuna_dependencies.json'))
...
def acquire():
    ...
    for name,digest in expected.items():h.require(h.sha(path/name)==digest,'pinned acquisition')

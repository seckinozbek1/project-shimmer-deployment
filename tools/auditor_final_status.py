...
B=Path(__file__).resolve().parents[1]/'docs/fix/auditor_final_run'
...
base=pathlib.Path('/home/ubuntu/shimmer-auditor-final/evidence')
for name in ['progress.json','training_status.json','status.json']:
    if (base/name).exists():print(name,(base/name).read_text())
p=base/'events.jsonl'
if p.exists():
    with p.open('rb') as f:
    ...
    print(json.dumps({'bytes':p.stat().st_size,'last':{k:last[k] for k in ['event','trial','update','example_id','checkpoint'] if k in last},'last_update':[{k:r[k] for k in ['update','ce','post_clip_norm','head_parameter_delta_norm','lora_parameter_delta_norm'] if k in r} for r in rows if r.get('event')=='update'][-1:], 'recent_milestones':[{'event':r['event'],'trial':r.get('trial'),'update':r.get('update'),'reason':r.get('reason'),'macro_f1':r.get('metrics',{}).get('macro_f1'),'min_recall':r.get('metrics',{}).get('minimum_class_recall'),'objective':r.get('objective')} for r in rows if r.get('event') in ['reuse_admitted','inner_val','numerical_prune']][-3:]}))
else:print('No final-workflow events yet')
"""
    import shlex

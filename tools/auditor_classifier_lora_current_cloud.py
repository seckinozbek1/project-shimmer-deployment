...
ROOT=Path(__file__).resolve().parents[1];BASE=ROOT/'docs/fix/auditor_classifier_lora_current_runtime_run';FROZEN=ROOT/'tuning/second_domain_agnostic_v2'
CREDENTIAL=Path('C:/Users/secki/local/api_keys/config.py');REMOTE='/home/ubuntu/shimmer-auditor-classifier-current'
...
def prepare():
    from auditor_classifier_lora_current_prepare import prepare as build
    build()
...
def execute():
    ...
    try:
        ...
        with (BASE/'LAUNCH_INTENT.json').open('x') as lock:json.dump(dict(epoch=start,maximum_instances=1,request_attachment='dcfd3146-30c0-488e-bfaa-43bf99428397'),lock)
        ...
        remote('pre_acquisition',prefix+exe+' -B tools/auditor_classifier_lora_current_remote.py preflight',120)
        remote('acquisition',prefix+exe+' -B tools/auditor_classifier_lora_current_remote.py acquire',1800)
        ...
        result=remote('classification_only',prefix+env+exe+' -B -u tools/auditor_classifier_lora_current_remote.py execute',22000,False)
        ...
        if iid:
            if ssh:
                try:
                    transport('stop_workloads',ssh+['pkill -TERM -f "[t]ools/auditor_classifier_lora_current_remote.py" || true'],30,False)
                    transport('process_final',ssh+['nvidia-smi --query-compute-apps=pid,process_name,used_memory --format=csv,noheader'],30,False)
                    transport('pack_evidence',ssh+['cd '+REMOTE+' && tar -czf evidence.tar.gz evidence execution_manifest.json training_authorization.json budget.json tools/auditor_classifier_lora_current_remote.py tools/auditor_classifier_lora_core.py tools/auditor_classifier_lora_fork.py tools/auditor_classifier_lora_stable.py tuning/auditor_classifier_lora/records.json tuning/auditor_classifier_lora/schedule.json tuning/auditor_classifier_lora/challenges.json tuning/auditor_classifier_lora_current_runtime/experiment.json tuning/auditor_classifier_lora_current_runtime/controls.json tools/auditor_classifier_lora_current_core.py'],120,False)

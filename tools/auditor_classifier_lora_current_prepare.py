...
def prepare():
    ...
    for p,h in c.read(D/'data_bindings.json').items():assert c.sha(ROOT/p)==h,p
    ...
    soft=2.5/rate*3600;hard=3.5/rate*3600;projected=900+1792*.5+60+32*.5+1792*.5+896*6+496*.5+240+600
    assert projected<hard and hard-600>projected-600,'Complete workload cannot fit with reserve'
    names=['tools/'+n for n in ['auditor_classifier_lora_current_remote.py','auditor_classifier_lora_current_core.py','auditor_classifier_lora_core.py','auditor_classifier_lora_fork.py','auditor_classifier_lora_stable.py','auditor_v2_execution.py','auditor_v2_diagnostic.py','auditor_linear_core.py']]
    ...
    names+=['tuning/auditor_classifier_lora_current_runtime/'+n for n in ['experiment.json','runtime_bindings.json','controls.json','data_bindings.json']]
    names+=list(c.read(D/'runtime_bindings.json'))
    commit=git('rev-parse','HEAD').decode().strip();blobs={}
    for name in sorted(set(names)):
        assert not any(x in name.lower() for x in ['holdout','producer','protected','features.npy','mean.npy','std.npy','head-200','train_baseline','auditor_v2_diagnostic_run'])
    ...
    permit=dict(action='current-runtime-rebase-classifier-only',operator_authorized=True,request_attachment='dcfd3146-30c0-488e-bfaa-43bf99428397',source_commit=commit,
    ...
    cloud=dict(source_commit=commit,bundle_sha256=c.sha(B/'runtime_bundle.zip'),instance=choice['metadata'],region=region,image=image,hourly_rate=rate,
        ...
        projected_seconds_with_reserve=projected,projected_cost=projected*rate/3600,name='shimmer-auditor-classifier-current')
    ...
    result=subprocess.run([sys.executable,'-B','tools/auditor_classifier_lora_current_remote.py','scope'],cwd=check,capture_output=True)

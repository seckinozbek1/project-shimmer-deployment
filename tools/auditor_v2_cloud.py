"""One authorized V2 head; $0.75 soft / $1.50 hard, no second instance."""
...
ROOT=Path(__file__).resolve().parents[1];BASE=ROOT/'docs/fix/auditor_v2_diagnostic_run';FROZEN=ROOT/'tuning/second_domain_agnostic_v2'
CREDENTIAL=Path('C:/Users/secki/local/api_keys/config.py');REMOTE='/home/ubuntu/shimmer-auditor-v2-diagnostic'
...
def prepare():
    from auditor_v2_prepare import prepare as build
    build()
...
def execute():
    ...
    def remote(label,command,timeout=60,required=True):
        ...
        if time.time()>=start+m['soft_seconds']:
            # Before feature extraction, bound all remaining mandatory compute
            # conservatively by 2400s; during execution the per-row remote gate
            # updates the actual remaining estimate and enforces the earlier cap.
            remaining_work=2400 if label not in ('classification_only',) else 2130
    ...
    try:
        ...
        with (BASE/'LAUNCH_INTENT.json').open('x') as lock:json.dump(dict(epoch=start,maximum_instances=1,request_attachment='1c5d2f2a-3534-4fc2-a2b8-0ac4a12f3536'),lock)
        ...
        remote('pre_acquisition',prefix+exe+' -B tools/auditor_v2_remote.py preflight',120)
        remote('acquisition',prefix+exe+' -B tools/auditor_v2_remote.py acquire',1800)
        ...
        result=remote('classification_only',prefix+env+exe+' -B -u tools/auditor_v2_remote.py execute',22000,False)
        ...
        if iid:
            if ssh:
                try:
                    transport('stop_workloads',ssh+['pkill -TERM -f "[t]ools/auditor_v2_remote.py" || true'],30,False)
                    transport('process_final',ssh+['nvidia-smi --query-compute-apps=pid,process_name,used_memory --format=csv,noheader'],30,False)
                    transport('pack_evidence',ssh+['cd '+REMOTE+' && tar -czf evidence.tar.gz evidence execution_manifest.json training_authorization.json budget.json tools/auditor_v2_remote.py tools/auditor_v2_execution.py tools/auditor_v2_diagnostic.py tools/auditor_linear_core.py tuning/auditor_v2_diagnostic adapter/adapter_config.json'],120,False)

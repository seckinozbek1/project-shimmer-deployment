"""One step120 classifier fork; $2.50 soft / $3.50 hard, one A10."""
...
ROOT=Path(__file__).resolve().parents[1];BASE=ROOT/'docs/fix/auditor_classifier_lora_stabilized_run';FROZEN=ROOT/'tuning/second_domain_agnostic_v2'
CREDENTIAL=Path('C:/Users/secki/local/api_keys/config.py');REMOTE='/home/ubuntu/shimmer-auditor-classifier-stabilized'
...
def prepare():
    from auditor_classifier_lora_stabilized_prepare import prepare as build
    build()
...
def execute():
    ...
    try:
        ...
        with (BASE/'LAUNCH_INTENT.json').open('x') as lock:json.dump(dict(epoch=start,maximum_instances=1,request_attachment='6234692b-4bf4-495e-952f-5d48d5219d6c'),lock)
        ...
        remote('pre_acquisition',prefix+exe+' -B tools/auditor_classifier_lora_stabilized_remote.py preflight',120)
        remote('acquisition',prefix+exe+' -B tools/auditor_classifier_lora_stabilized_remote.py acquire',1800)
        ...
        result=remote('classification_only',prefix+env+exe+' -B -u tools/auditor_classifier_lora_stabilized_remote.py execute',22000,False)
        require(result.returncode==0,'Evaluation runtime failed; no rescue or second instance')
        ...
        if iid:
            if ssh:
                try:
                    transport('stop_workloads',ssh+['pkill -TERM -f "[t]ools/auditor_classifier_lora_stabilized_remote.py" || true'],30,False)
                    transport('process_final',ssh+['nvidia-smi --query-compute-apps=pid,process_name,used_memory --format=csv,noheader'],30,False)
                    transport('pack_evidence',ssh+['cd '+REMOTE+' && tar -czf evidence.tar.gz evidence execution_manifest.json training_authorization.json budget.json tools/auditor_classifier_lora_stabilized_remote.py tools/auditor_classifier_lora_core.py tools/auditor_classifier_lora_fork.py tools/auditor_classifier_lora_stable.py tuning/auditor_classifier_lora/records.json tuning/auditor_classifier_lora/schedule.json tuning/auditor_classifier_lora/challenges.json tuning/auditor_classifier_lora_fork/experiment.json tuning/auditor_classifier_lora_fork/update0_controls.json tuning/auditor_classifier_lora_fork/train_baseline.json'],120,False)
                    evidence_hash=transport('evidence_hash',ssh+['sha256sum '+REMOTE+'/evidence.tar.gz'],30)
                    transport('evidence_download',['scp',*options,host+':'+REMOTE+'/evidence.tar.gz',str(BASE/'evidence.tar.gz')],300)

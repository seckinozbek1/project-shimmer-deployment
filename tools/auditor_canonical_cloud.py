"""One authorized fresh Auditor canonical training instance; no Producer or protected work."""
...
ROOT=Path(__file__).resolve().parents[1];BASE=ROOT/'docs/fix/auditor_canonical_tuning_run';FROZEN=ROOT/'tuning/second_domain_agnostic_v2'
CREDENTIAL=Path('C:/Users/secki/local/api_keys/config.py');REMOTE='/home/ubuntu/shimmer-auditor-canonical'
...
def prepare():
    from auditor_canonical_prepare import prepare as build
    build()
...
def execute():
    ...
    try:
        ...
        write('LAUNCH_INTENT.json',dict(epoch=start,maximum_instances=1,request_attachment='ea56d7c9-febe-421d-9f5d-817905c15c81'))
        ...
        remote('pre_acquisition',prefix+exe+' -B tools/auditor_canonical_remote.py preflight',120)
        remote('acquisition',prefix+exe+' -B tools/auditor_canonical_remote.py acquire',1800)
        ...
        result=remote('auditor_training',prefix+env+exe+' -B -u tools/auditor_canonical_remote.py execute',22000,False)
        ...
        if iid:
            if ssh:
                try:
                    transport('stop_workloads',ssh+['pkill -TERM -f "[t]ools/auditor_canonical_remote.py" || true'],30,False)
                    transport('process_final',ssh+['nvidia-smi --query-compute-apps=pid,process_name,used_memory --format=csv,noheader'],30,False)
                    transport('pack_evidence',ssh+['cd '+REMOTE+' && tar -czf evidence.tar.gz evidence execution_manifest.json training_authorization.json budget.json tools/auditor_canonical_remote.py tuning/auditor_canonical_execution'],120,False)

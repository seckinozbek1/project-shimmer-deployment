...
ROOT=Path(__file__).resolve().parents[1];BASE=ROOT/'docs/fix/producer_checkpoint120_eval_runtime_v2';FROZEN=ROOT/'tuning/second_domain_agnostic_v2'
CREDENTIAL=Path('C:/Users/secki/local/api_keys/config.py');REMOTE='/home/ubuntu/shimmer-checkpoint120-eval'
...
def prepare():
    from checkpoint120_prepare import prepare as build
    build()
...
def execute():
    ...
    try:
        ...
        write('LAUNCH_INTENT.json',dict(epoch=start,maximum_instances=1,request_attachment='0a58459f-6387-45d5-8525-9965c91848ba'))
        ...
        remote('pre_acquisition',prefix+exe+' -B tools/checkpoint120_remote.py preflight',120)
        remote('acquisition',prefix+exe+' -B tools/checkpoint120_remote.py acquire',1800)
        ...
        result=remote('producer_evaluation',prefix+env+exe+' -B -u tools/checkpoint120_remote.py execute',7200,False)
        ...
        if iid:
            if ssh:
                try:
                    transport('stop_workloads',ssh+['pkill -TERM -f "[t]ools/checkpoint120_remote.py" || true'],30,False)
                    transport('process_final',ssh+['nvidia-smi --query-compute-apps=pid,process_name,used_memory --format=csv,noheader'],30,False)
                    transport('pack_evidence',ssh+['cd '+REMOTE+' && tar -czf evidence.tar.gz evidence execution_manifest.json evaluation_authorization.json budget.json tools/checkpoint120_remote.py tuning/second_tuning_eval_runtime_v2/freeze.json'],120,False)

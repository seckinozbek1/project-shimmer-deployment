"""One authorized Auditor linear probe; hard 50/55-minute deadline."""
...
ROOT=Path(__file__).resolve().parents[1];BASE=ROOT/'docs/fix/auditor_linear_probe_run';FROZEN=ROOT/'tuning/second_domain_agnostic_v2'
CREDENTIAL=Path('C:/Users/secki/local/api_keys/config.py');REMOTE='/home/ubuntu/shimmer-auditor-linear-probe'
...
def prepare():
    from auditor_linear_prepare import prepare as build
    build()
...
def execute():
    ...
    def transport(label,argv,timeout=60,required=True):
        if iid:
            timeout=min(timeout,max(1,int(start+3180-time.time())))
        begin=time.time()
    ...
    try:
        ...
        write('LAUNCH_INTENT.json',dict(epoch=start,maximum_instances=1,request_attachment='9f0ba690-8742-4328-9662-7a330e3321db'))
        ...
        remote('pre_acquisition',prefix+exe+' -B tools/auditor_linear_remote.py preflight',120)
        remote('acquisition',prefix+exe+' -B tools/auditor_linear_remote.py acquire',1800)
        ...
        write('budget.json',dict(launch_epoch=start,workload_deadline_epoch=deadline,termination_deadline_epoch=start+3300,soft_epoch=start+m['soft_seconds'],hard_epoch=start+m['hard_seconds'],hourly_rate=m['hourly_rate']))
        transport('budget_transfer',['scp',*options,str(BASE/'budget.json'),host+':'+REMOTE+'/budget.json'],30)
        result=remote('linear_probe',prefix+env+exe+' -B -u tools/auditor_linear_remote.py execute',22000,False)
        ...
        if iid:
            if ssh:
                try:
                    transport('stop_workloads',ssh+['pkill -TERM -f "[t]ools/auditor_linear_remote.py" || true'],30,False)
                    transport('process_final',ssh+['nvidia-smi --query-compute-apps=pid,process_name,used_memory --format=csv,noheader'],30,False)
                    transport('pack_evidence',ssh+['cd '+REMOTE+' && tar -czf evidence.tar.gz evidence execution_manifest.json training_authorization.json budget.json tools/auditor_linear_remote.py tools/auditor_linear_core.py tuning/auditor_linear_probe adapter/adapter_config.json'],120,False)

"""One authorized fresh Producer V3 training instance; no Auditor or protected work."""
...
ROOT=Path(__file__).resolve().parents[1];BASE=ROOT/'docs/fix/producer_tuning_v3_run';FROZEN=ROOT/'tuning/second_domain_agnostic_v2'
CREDENTIAL=Path('C:/Users/secki/local/api_keys/config.py');REMOTE='/home/ubuntu/shimmer-producer-v3'
...
def prepare():
    from producer_v3_prepare import prepare as build
    build()
...
def execute():
    ...
    try:
        ...
        write('LAUNCH_INTENT.json',dict(epoch=start,maximum_instances=1,request_attachment='3022274b-cb22-4d6c-a0c0-3842d1ad0952'))
        ...
        remote('pre_acquisition',prefix+exe+' -B tools/producer_v3_remote.py preflight',120)
        remote('acquisition',prefix+exe+' -B tools/producer_v3_remote.py acquire',1800)
        ...
        write('budget.json',dict(launch_epoch=start,workload_deadline_epoch=deadline,soft_epoch=start+m['soft_seconds'],hard_epoch=start+m['hard_seconds'],hourly_rate=m['hourly_rate']))
        transport('budget_transfer',['scp',*options,str(BASE/'budget.json'),host+':'+REMOTE+'/budget.json'],30)
        result=remote('producer_training',prefix+env+exe+' -B -u tools/producer_v3_remote.py execute',22000,False)
        ...
        if iid:
            if ssh:
                try:
                    transport('stop_workloads',ssh+['pkill -TERM -f "[t]ools/producer_v3_remote.py" || true'],30,False)
                    transport('process_final',ssh+['nvidia-smi --query-compute-apps=pid,process_name,used_memory --format=csv,noheader'],30,False)
                    transport('pack_evidence',ssh+['cd '+REMOTE+' && tar -czf evidence.tar.gz evidence execution_manifest.json training_authorization.json budget.json tools/producer_v3_remote.py tuning/producer_v3/freeze.json tuning/producer_v3/experiment.json tuning/producer_v3/split.json tuning/producer_v3/execution_plan.json tuning/second_tuning_eval_runtime_v2_1/freeze.json'],120,False)
...
def status():
    for name in ('CURRENT_PHASE.json','cost.json','producer_evaluation_timing.json','control_summary.json','TERMINATION_VERIFIED.json','controller_failure.json'):

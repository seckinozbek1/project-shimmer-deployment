...
work: entry main and origin/main both `5b72b16c67b02890ffc8be39096b5bad10e1c05c`.
This item closes in the local commit `Record local performance diagnostic baseline`,
one commit ahead of that unchanged tracking ref. No fetch, push or history rewrite.
The locked next item is exactly **Local speed-optimization audit**; it has not begun.

[LOCAL_PERFORMANCE_DIAGNOSTIC.md](LOCAL_PERFORMANCE_DIAGNOSTIC.md) contains the
timing map recorded before instrumentation, workload validity, measurements,
resource caveats, behavioral proofs and ranked candidates. Only that report,
RESUME and this new ledger entry are tracked changes. Diagnostic helpers and
JSON/JSONL evidence remain ignored under `output/local_performance/`.

One corrected controlled native baseline used unchanged shipped `clinical_reference`
material: three files / 5,265 bytes, one target / six units, all five conventions,
explicit standalone target/grounding roles and Normal paired Review. No model,
budget, scheduling, governance or production behavior changed. No answer key was
opened/scored and no operator/private document was reviewed. No provider, paid API
or remote VM was used. Only one heavy workload ran; all processes have exited.

Native wall **5,054.4 s (84 min 14.4 s)**, exit 0, actual completion reached_end
true, one document / three amendments. All **26** dispatches, generations, cost
rows and evidence rows reconcile, with no open spans. Generation **4,947.4 s
(97.9%)**: producer 13 calls / 3,040.1 s, auditor 13 / 1,907.3 s. Phase ranking:
production 38.73 min, convention review 16.49 min, verification 15.55 min,
editorial 12.43 min. First producer/auditor loads 12.843 / 9.423 s, one producer
reload 12.163 s, 23 resident hits. Encoding 55.422 s; deterministic arithmetic,
indexing and rendering are minor on this workload.

Peak RSS 6.67 GiB, board VRAM 7.51 of 8 GiB, system available RAM minimum
115.5 MiB. Producer/auditor GPU utilization medians 82% / 34%. Early software
thermal slowdown and later CPU/GPU clock variation are measured; causality of
the later underutilization is not established. No machine settings changed.
The result is a measured laptop baseline, not a controlled repeatability estimate.

Twelve calls hit caps; CITATION_RESOLVER and VERIFIER wrote contract-violation
dumps. The board still reports REVIEWED, one observation, no failed rank. A
cached-tokenizer-only replay through the actual contract parser matches the saved
runtime result after 112 retokenized tokens of 8,192 generated by the clerk in
733.4 s. It leaves a large unconsumed suffix. The corrected fixture verifies the
contract map and neutralize/fail/restore/pass. Safe early stopping and seconds
saved remain unproven; no parser, stopping rule or cap was changed.

Warm derived-store reuse was independently exercised in 0.143 s for 11 passages,
with zero model imports/loads and unchanged hash. Neutralizing staleness reached
the cold import path and failed before weights; restoration reused the store.
Consumer timing/attribution/payload/interruption fixtures pass, as do Windows
loopback-versus-egress and overlapping-wall/error-reducer fixtures. An ignored
reducer bug for failed generations inside an open dispatch was fixed; the complete
baseline was unaffected. The initial parser probe's contract-map fixture was
corrected before accepting its finding. No production correctness fix was needed.

A preliminary 62.2 s attempt made no generation: the diagnostic guard rejected
Windows asyncio IPC. Its evidence and original harness remain in `attempt_guard/`;
the corrected guard fixture passes and the new isolated baseline completed.
One Hugging Face metadata HTTP API entry was blocked in that baseline and the
loader recovered locally. This proves no successful egress, not zero attempted
HTTP requests. The prior unmounted image's zero-attempt model probe is separate.

All 128 copied runtime files and three synthetic inputs match their initial
hashes. All 18 monitored operator-state files remain unchanged, none added or
missing. Existing Docker/BuildKit and model caches were preserved. Intentional
untracked handoff/durable state is excluded from the commit.

No shipped/runtime change means the operator's report-only exception applies:
**no new full host gate or image rebuild required or run**. No README behavior
debt. The prior startup host gate remains 256 PASS / 0 WARN / 2 SKIP / 0 FAIL,
exit 0. The unchanged `shimmer:startup` image remains
`sha256:90ce701581520e4ec763d5d36d22d59f1120668200a76a820af156de0aca6356`;
its historical unmounted gate remains 246 PASS / 10 SKIP / 2 FAIL (missing intake
checks 28/31), exit 1. These were not rerun or relabeled for this item.

Cold download/disk-cache cost, fully warm repeated pipeline, separate prefill/decode,
human latency, prior versions, Sensitive/Draft and mature ontology were not measured.
Historical README timings use older conditions and are not equivalent comparisons.
Candidates are ranked for the next audit only: output consumption, host variability,
legal-follow-up value, context/prefill, embedding reuse, then low-impact reload/I/O
work. No speed optimization, README/routing/UI audit, FinOps/VM, fresh-clone or
release-readiness work started. Roadmap order and conditional VM item are intact.

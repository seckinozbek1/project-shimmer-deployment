...
RUNTIME = ROOT / 'tuning/second_tuning_eval_runtime_v2'
OUT = ROOT / 'evidence'
...
def scope():
    ...
    records = read(RUNTIME/'prepared_dev.json')
...
def execute():
    ...
    def run(label,record,reference=False,probe=False):
        ...
        sink=ev.DurableRows(OUT/(label+'.jsonl'))
        try:
            if probe:
    ...
    try:
        ...
        sink=ev.DurableRows(OUT/'full_dev.jsonl')

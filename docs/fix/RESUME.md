...
LOCAL_TARGET_INDETERMINATE. Real-model latency and semantic quality are unmeasured.
The host import probe fails with duplicate OpenMP runtime initialization before
model loading. The alternate virtual environment lacks Transformers/tokenizers.
No unsafe override, environment rebuild or download was attempted.

Validation: 49 PASS / 1 SKIP / 0 FAIL in the ordinary safe gate; 17 new mechanism
checks, 23 existing topology checks, 9 safe legacy checks. The skip is optional
...
Next evidence, in order: safely resolve inference initialization, run a bounded
native-template/source-span semantic A/B probe, measure independent audit quality
and residency/concurrency, fill projection inputs. A full ordinary local run needs

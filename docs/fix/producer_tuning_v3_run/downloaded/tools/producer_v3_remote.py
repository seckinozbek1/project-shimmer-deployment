"""One prospectively authorized fresh Producer V3 run; no resume or alternate flow."""
...
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/'evidence';V3=ROOT/'tuning/producer_v3';RT=ROOT/'tuning/second_tuning_eval_runtime_v2_1'
def boundary(event,args):
    if event=='open' and isinstance(args[0],(str,bytes,os.PathLike)):
        p=Path(os.fsdecode(args[0])).resolve()
        if 'auditor' in {s.lower() for s in p.parts} or 'protected' in p.name.lower():raise PermissionError('Forbidden Auditor/protected artifact')
...
def scope():
    ...
    assert sha(V3/'freeze.json')==permit['v3_freeze_sha256']==m['v3_freeze_sha256']
    assert sha(V3/'dataset.json')==permit['dataset_sha256']=='b036707820a1851e94432319cc340cafec10a1b6093fab39b573cc4a47b9ecf0'
    assert sha(RT/'freeze.json')==permit['runtime_release_sha256']=='8d1fd41d1ab059e47b1297c1c39cd29711acd8fa5446a60780f9a97faef1d1fa'
    assert permit['operator_authorized'] and permit['action']=='producer-v3-fresh-training' and permit['maximum_instances']==1 and permit['hard_budget_usd']==6
    assert not permit['auditor'] and not permit['protected_access'] and permit['fresh_adapter'] and permit['gpu_type']=='gpu_1x_a10'
    ...
    assert len(rows)==420 and all(x['role']=='producer' for x in rows)
    assert spec['dataset_sha256']==sha(V3/'dataset.json') and spec['split_sha256']==sha(V3/'split.json')
    assert spec['checkpoint_steps']==[84,168] and spec['training']['max_steps']==168
    assert len(split['train'])==336 and len(split['validation'])==84
    from collections import Counter
    assert len(plan['optimizer_schedule'])==168
    ...
    ev.validate_records(records,protocol);ev.validate_generation(protocol['generation_kwargs'],protocol)
    # Check every uploaded frozen member against the actual V3 freeze without
    # uploading the historical/protected/Auditor dependencies of the full release.
    frozen=read(V3/'freeze.json')
    for name,digest in dict(frozen['files'],**frozen['dependency_files']).items():
        if name in m['files']:assert sha(ROOT/name)==digest,name
...
def per_pass_speed(first,second,minimum):
    ...
    for rows in (first,second):
        assert len(rows)==6 and all(math.isfinite(x['generation_seconds']) and x['generation_seconds']>0 for x in rows)
...
def execute():
    ...
    assert max(len(x['input_ids']) for x in encoded.values())==988
    assert all(len(x['input_ids'])<=992 and x['target_length']<=288 and all(v==-100 for v in x['labels'][:x['prompt_length']]) for x in encoded.values())
    for row in records:
    ...
    assert model.config.model_type=='qwen2' and model.is_loaded_in_4bit
    ...
    assert sum(p.numel() for p in params)==spec['architecture']['adapter_parameters']==20185088
    ...
    def save_checkpoint(step):
        ...
        identity=dict(experiment='producer-targeted-v3',step=step,files={name:sha(directory/name) for name in ('adapter_config.json','adapter_model.safetensors')})
        write(f'checkpoint-{step}/identity.json',identity);return identity
    initial=save_checkpoint(0)
    write('initialization.json',dict(seed=7,fresh_lora=True,old_adapter_loaded=False,lora_B_all_zero=True,adapter=initial,model_load_seconds=time.time()-begin,trainable_parameters=20185088))
    ...
    # Import only the frozen pure sink/control definitions; local_common's
    # tokenizer-only guard must not be installed inside the authorized executor.
    tree=ast.parse((V3/'runtime_binding.py').read_text());nodes=[x for x in tree.body if isinstance(x,(ast.ClassDef,ast.FunctionDef)) and x.name in ('V3RawSink','optimized_control_gate')]
    namespace=dict(runtime=ev,hashlib=hashlib);exec(compile(ast.Module(body=nodes,type_ignores=[]),'<frozen V3 binding>','exec'),namespace)
    Sink=namespace['V3RawSink'];control_gate=namespace['optimized_control_gate']
    ...
    sampler=subprocess.Popen([sys.executable,str(RT/'telemetry_worker.py'),'--pid',str(os.getpid()),'--output',str(OUT/'telemetry.jsonl'),'--stop-file',str(OUT/'telemetry.stop')])
    ...
    def progress(phase):
        write('progress.json',dict(phase=phase,step=completed_updates,eval_outputs=eval_outputs,epoch=time.time(),training_seconds=training_wall,
            remaining_updates=168-completed_updates,remaining_generations=196-eval_outputs,
    ...
    def budget_gate(phase):
        ...
        rate=min(protocol['minimum_prefill_inclusive_tokens_per_second'],generated_tokens/generation_wall) if generation_wall else protocol['minimum_prefill_inclusive_tokens_per_second']
        remaining=(168-completed_updates)*step_rate+(196-eval_outputs)*288/rate
    ...
    def evaluate(step,identity):
        ...
        admitted['per_pass_tokens_per_second']=per_pass_speed(first,second,cp['minimum_prefill_inclusive_tokens_per_second'])
        write(f'checkpoint-{step}/control_admission.json',admitted);budget_gate(f'checkpoint-{step}:admitted')
        # Persist one exclusive file across all84 rows; per-row progress/budget,
        ...
        assert len(results)==84 and [x['example_id'] for x in results]==cp['dev_ids']
        ...
        checkpoint=dict(step=step,metrics=metrics,adapter_sha256=identity['files']['adapter_model.safetensors'],identity=identity)
        ...
        # and exact state restoration. This permits a fresh lifecycle at168,
        # never retry of a failed checkpoint or resume of partial output.
        ev._PREFLIGHTS.pop(model)
        write(f'checkpoint-{step}/status.json',dict(complete=True,rows=84,passes=selection.passes(metrics,spec['selection_gates'])))
    try:
        ...
        assert completed_updates==168 and len(checkpoints)==2 and eval_outputs==196
        chosen=selection.select(checkpoints,spec);write('selection.json',chosen)
        if chosen['verdict']=='PRODUCER_V3_PASS':
            import shutil
            ...
            verdict='PRODUCER_TUNING_V3_PASS'
        else:verdict='PRODUCER_TUNING_V3_FAIL'
        write('status.json',dict(verdict=verdict,training_updates=168,checkpoint_rows={'84':84,'168':84},training_seconds=training_wall,selection=chosen))
    except BaseException as exc:
        write('status.json',dict(verdict='PRODUCER_TUNING_V3_INDETERMINATE',step=completed_updates,error_type=type(exc).__name__,reason=str(exc)))
        raise
    finally:
    ...
    while True:
        ...
        if now>=budget['soft_epoch']:
            try:
                ...
                rate=min(4.438538339317307,progress.get('measured_generation_tokens_per_second') or 4.438538339317307)
                remaining=progress['remaining_updates']*step_rate+progress['remaining_generations']*288/rate
        ...
        if stop:
            write('budget_stop.json',dict(epoch=now,verdict='PRODUCER_TUNING_V3_INDETERMINATE',projected_remaining_seconds=remaining));os.kill(pid,signal.SIGTERM);return
        time.sleep(5)

...
| `GET` | `/runs/{run_id}` | token | api STEP B1/B2/B3: one run's complete record in a single call: `state` (`queued` / `running` / `awaiting_approval` / `stopped` / `cancelled`), `outcome` (populated once `state` is `stopped` or `cancelled`: `succeeded` / `governance_stop` / `crashed` / `timed_out` / `cancelled`), `stop_reason`, `pending_approval` (populated once `state` is `awaiting_approval`: `topic`, `message`, `payload`, `asked_at`, `default_on_timeout`, `timeout_at` — the full pending question and what happens if nobody answers, not just its existence), `documents` (per-document status and deliverables path; nothing is deleted by a cancel, so partial output stays visible here), plus the fields `/status` already returned. The deliverables route described in the design doc is not yet built; see `docs/api/SURFACE_DESIGN.md`. |

...
| `GET` | `/runs/{run_id}` | token | api STEP B1: one run's complete record in a single call: `state` (`queued` / `running` / `awaiting_approval` / `stopped` / `cancelled`), `outcome` (populated once `state` is `stopped`: `succeeded` / `governance_stop` / `crashed` / `timed_out`), `stop_reason`, `pending_approval` (populated once `state` is `awaiting_approval`: the full pending question, not just its existence), `documents` (per-document status and deliverables path), plus the fields `/status` already returned. `cancelled` and the deliverables/approval-answering routes described in the design doc are not yet built; see `docs/api/SURFACE_DESIGN.md`. |
| `GET` | `/results/{run_id}` | token | A zip of the deliverables (400 before the run completes). |
| `GET` | `/approvals` | token | Every run currently awaiting a governed decision. |
| `POST` | `/approvals/{run_id}` | token | Records a human's decision. Body `decision` + `rationale`; writes `<run>/audit/approval_decision.json` atomically and nothing else, it never itself evaluates the decision. |

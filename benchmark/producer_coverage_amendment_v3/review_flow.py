...
WORK=s.ROOT/'output/producer_coverage_amendment_v3'
def policy_intact():
 f=s.read(s.HERE/'PRE_REVIEW_METHOD_FREEZE.json')
...
def validate(v,p,slot):
 required={'protocol','policy','evidence_kind','use','packet_id','input_sha256','slot','provenance','reviewed_at','label','rationale'}
 ...
 if v.get('evidence_kind')!='training_label_curation_evidence' or v.get('use')!='curation_only':raise ValueError('Curation-only provenance required')
...
def collect():
 policy_intact();ps=packets();selection=s.read(s.HERE/'population.json');required=set(selection['hard_packet_ids']);records={};invalid=[]
 for slot in 'ABC':
...
def freeze():
 ...
 for name in ('PRE_REVIEW_METHOD_FREEZE.json','population.json','PRE_GOLD_ACCESS_AUDIT.json'):shutil.copyfile(s.HERE/name,dest/name)
 ...
 import method_freeze
 s.write(dest/'PRE_GOLD_ACCESS_AUDIT.json',method_freeze.audit())
 shutil.copytree(WORK/'access_logs',dest/'access_logs')
 s.write(dest/'isolation.json',dict(evidence_kind='training_label_curation_evidence',use='curation_only',fresh_primary_contexts=3,fresh_adjudicator=True,detected_violations=0,method='Exact packet hashes, folder-only instructions and agent attestations; shared filesystem/parent model, no OS enforcement or cross-model diversity',human_reviewed=False))
 s.write(dest/'PRE_GOLD_FREEZE.json',dict(protocol=s.PROTOCOL,evidence_kind='training_label_curation_evidence',use='curation_only',frozen_at=datetime.now(timezone.utc).isoformat(),primaries=len(ps)*3,repairs=sum(len(list((dest/slot/'repairs').glob('*.json'))) for slot in 'ABC'),adjudications=len(required),hashes={p.relative_to(dest).as_posix():s.digest(p.read_bytes()) for p in dest.rglob('*') if p.is_file()}))
 manifest=s.read(dest/'PRE_GOLD_FREEZE.json')
 manifest.update(policy_id=s.POLICY,release_id=s.PROTOCOL,active_release=s.read(s.HERE/'ACTIVE_RELEASE.json'))
 bound=[p for p in s.HERE.rglob('*') if p.is_file() and p!=dest/'PRE_GOLD_FREEZE.json' and '__pycache__' not in p.parts and 'post_freeze' not in p.parts]
 manifest['files']={p.relative_to(s.ROOT).as_posix():s.digest(p.read_bytes()) for p in bound}
 s.write(dest/'PRE_GOLD_FREEZE.json',manifest)

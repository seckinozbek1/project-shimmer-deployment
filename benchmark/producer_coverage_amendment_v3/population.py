...
def main():
 ...
 for r in rows:
  r['packet_id']='producer-v3-'+s.digest((SEED+r['example_id']).encode())[:24];pid=r['packet_id'];inp=inputs[r['example_id']]
...
def create_workspaces():
 hashes=s.read(s.HERE/'population.json')['packet_hashes']
 for slot in 'ABC':
  folder=WORK/('reviewer_'+slot);(folder/'packets').mkdir(parents=True,exist_ok=True);(folder/'reviews').mkdir(exist_ok=True)

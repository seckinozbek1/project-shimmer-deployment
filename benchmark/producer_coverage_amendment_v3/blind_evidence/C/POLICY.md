# producer-semantic-policy-v3

Amendment: producer-coverage-amendment-v3. Renderer: producer-target-renderer-v3.
...
## V3-only structural amendment and strict phases
The only semantic-method change from V2 is structural segmentation. `structural.py` prospectively defines source-exact segments. Line/table boundaries and colon labels are structural positions; finite copulas/verbs, explicit questions and CLM identifiers identify content-bearing propositions. Structural-only labels never donate epistemic words to a neighboring proposition. No phrase-specific exceptions, similarity or new ontology attributes are introduced. A proposition retains all negation and modality even in a heading position. Content outside the finite semantic ontology is a review ambiguity, never silently deemed structural.

Phases: 0 generic method design; 1 committed method freeze; 2 packet-only primary review; 3 consensus/adjudication/canonical rendering; 4 committed complete label freeze; 5 explicitly authorized target comparison; 6 candidate/readiness calculation. Every maintained V3 command installs the access guard before task imports. Post-gold access requires the exact active V3 release, exact committed label manifest at a full 40-character commit, unchanged freeze hashes and an explicit matching phase marker. Broad historical gates are POST_GOLD_ONLY. No authored target-bearing read or test is permitted pre-freeze. Fresh reviewers have no project conversation/history and receive only their assigned isolated packet workspace. Parent has historical aggregate knowledge but does not author labels.

...
def main():
 ...
 recorded=git(s.ROOT,'show',args.pre_gold_commit+':benchmark/producer_coverage_amendment_v1/blind_evidence/PRE_GOLD_FREEZE.json')
 ...
 s.write(out/'GOLD_COMPARISON_STARTED.json',dict(started_at=datetime.now(timezone.utc).isoformat(),pre_gold_commit=args.pre_gold_commit,freeze_sha256=s.digest((s.HERE/'blind_evidence/PRE_GOLD_FREEZE.json').read_bytes())))
 # First authored semantic target access for this amendment is after the checkpoint.
 ...
 labels=s.read(s.HERE/'blind_evidence/labels.json');canonical=s.read(s.HERE/'blind_evidence/canonical_targets.json');cons=s.read(s.HERE/'blind_evidence/consensus.json');sel=s.read(s.HERE/'selection.json');excluded_reserve=set(s.read(s.HERE/'selection_compliance.json')['disallowed_reserve_ids']);packets=f.packets()
 comparisons={};counters=Counter();train=[];dev=[];exclusions=[]
 for row in sel['rows']:
  ...
  if agrees:classification='agreement';resolved=True
   ...
   if gm<mm and extras and all(source_settles_extra(a,packet) for a in extras):classification='legacy_authored_target_under_specification';resolved=True
  ...
  result.update(classification=classification,material_dispute_resolved=resolved,eligible=False)
  reasons=[]
  ...
  if row['example_id'] in excluded_reserve:reasons.append('prohibited_easy_reserve')
  ...
  if not reasons:
   ...
   item=dict(example_id=row['example_id'],packet_id=pid,split=row['split'],role='producer',domain=row['domain'],template_family=row['template_family'],document_family=row['document_family'],
    provenance='machine_adjudicated_producer_candidate_v1',human_reviewed=False,policy=s.POLICY,renderer=s.RENDERER,input=packet,input_sha256=packets[pid]['input_sha256'],typed_label=label,canonical_target=target)
 ...
 s.write(out/'legacy_comparison.json',comparisons);s.write(out/'legacy_summary.json',dict(total=len(comparisons),**counters));s.write(out/'excluded_candidates.json',exclusions)
 access=s.HERE/'machine_adjudicated_producer_candidates_v1'
 for split,rows in [('train',train),('dev',dev)]:s.write(access/(split+'.json'),rows)
 s.write(access/'manifest.json',dict(policy=s.POLICY,protocol=s.PROTOCOL,provenance='machine_adjudicated_producer_candidate_v1',human_reviewed=False,training_authorized=False,
 ...
 summary=dict(protocol=s.PROTOCOL,packets=len(labels),splits=dict(Counter(r['split'] for r in sel['rows'])),primary_expected=3*len(labels),primary_completed=sum(len(list((s.HERE/'blind_evidence'/slot/'reviews').glob('*.json'))) for slot in 'ABC'),
  ...
  candidates=dict(train=len(train),dev=len(dev)),legacy=dict(counters),human_reviewed=False,training_authorized=False)
